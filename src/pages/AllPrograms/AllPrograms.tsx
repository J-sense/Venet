...
export default function AllPrograms() {
  const { data: allProgramsData, isLoading, isError } = useAllProgramsQuery(undefined);
  const programs: any[] = allProgramsData?.data ?? [];
  ...
  const cartTitles = rawCartItems.map((item: any) =>
    ...
    <div className="w-full min-h-screen bg-black py-24 md:py-44 px-6 relative overflow-hidden flex items-center">
      <div className="max-w-[1000px] w-full mx-auto relative z-10">
        ...
        <div className="mb-16 text-center">
          <h2 className="text-white text-4xl font-bold mb-3">All Programs</h2>
          <p className="text-slate-400">
            Explore all available programs and find the one that fits you best.
          </p>
        </div>

        {isLoading && (
          <p className="text-slate-400 text-center">Loading programs...</p>
        )}
        {isError && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {programs.map((program: any, idx: number) => {
            ...
            const isAddedInRedux = cartTitles.some(
              (t: string) => t.toLowerCase() === program?.name?.toLowerCase(),
            );
            const isAdded = token
